...
{
    this->index = 0;
    this->transfers = QMap<int, AMQPTransfer *>();
}
...
AMQPTransfer *AMQPTransferMap::addTransfer(AMQPTransfer *item)
{
    ...
    this->transfers.insert(this->index, item);
...
{
    AMQPTransfer *item = this->transfers.value(Id);
...
QMap<int, AMQPTransfer *> AMQPTransferMap::getTransfers() const
{
    return transfers;
}
