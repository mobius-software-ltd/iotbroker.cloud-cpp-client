...
TLVAMQP *AMQPData::getValue()
{
    ...
    if (!this->data.getByteArray().isNull()) {
        bin = AMQPWrapper::wrapBinary(this->data.getByteArray());
    } else {
        bin = new AMQPTLVNull();
    }
...
{
    if (!value->isNull()) {
        this->data = ByteArray(AMQPUnwrapper::unwrapBinary(value));
    }
}

ByteArray AMQPData::getData() const
{
    return data;
}

void AMQPData::setData(const ByteArray &value)
{
    data = value;
}
