...
AMQPTLVList *AMQPSASLChallenge::arguments()
{
    ...
    if (this->challenge.getByteArray().isNull()) {
        qDebug() << "AMQPSASLChallenge::arguments::challenge";
        return new AMQPTLVList();
    }

    list->addElementWithIndex(0, AMQPWrapper::wrapBinary(this->challenge.getByteArray()));
...
void AMQPSASLChallenge::fillArguments(AMQPTLVList *list)
{
    ...
    if (size == 0) {
        qDebug() << "AMQPSASLChallenge::fillArguments::size == 0";
        return;
    }
    if (size > 1) {
        qDebug() << "AMQPSASLChallenge::fillArguments::wrong_size";
        return;
    }
    if (size > 0) {
        TLVAMQP *element = list->getList().at(0);
        if (element->isNull()) {
            qDebug() << "AMQPSASLChallenge::fillArguments::challenge";
            return;
        }
...
ByteArray AMQPSASLChallenge::getChallenge() const
{
    return challenge;
}

void AMQPSASLChallenge::setChallenge(const ByteArray &value)
{
    challenge = value;
}
