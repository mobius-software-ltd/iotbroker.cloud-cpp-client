...
void AMQPDetach::fillArguments(AMQPTLVList *list)
{
    ...
    if (size == 0) {
        qDebug() << "AMQPDetach::fillArguments::size == 0";
        return;
    }
    if (size > 3) {
        qDebug() << "AMQPDetach::fillArguments::wrong size";
        return;
    }
    if (size > 0) {
        TLVAMQP *element = list->getList().at(0);
        if (element->isNull()) {
            qDebug() << "AMQPDetach::fillArguments::handle";
            return;
        }
    ...
    if (size > 2) {
        ...
        if (!element->isNull()) {
            if (element->getTypeValue() != AMQP_LIST0_TYPE && element->getTypeValue() != AMQP_LIST8_TYPE && element->getTypeValue() != AMQP_LIST32_TYPE) {
                qDebug() << "AMQPDetach::fillArguments::wrong element type";
                return;
            }
...
QVariant *AMQPDetach::getHandle() const
{
    return handle;
}

void AMQPDetach::setHandle(QVariant *value)
{
    handle = value;
}

QVariant *AMQPDetach::getClosed() const
{
    return closed;
}

void AMQPDetach::setClosed(QVariant *value)
{
    closed = value;
}
