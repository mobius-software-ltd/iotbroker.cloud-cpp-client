...
SNRegister::SNRegister(int topicID, int packetID, QString topicName)
{
...
int SNRegister::getLength()
{
    if (this->topicName.length() == 0) {
        qDebug() << "SNRegister must contain a valid topic name";
        return 0;
    }

    int length = 6;
    length += this->topicName.length();
...
QString SNRegister::getTopicName() const
{
    return topicName;
}

void SNRegister::setTopicName(const QString &value)
{
    topicName = value;
}
