...
SNConnect::SNConnect(bool cleanSession, int duration, QString clientID, bool willPresent) : SNConnect()
{
...
int SNConnect::getLength()
{
    if (this->clientID.length() <= 0) {
        qDebug() << "connect must contain a valid clientID";
        return 0;
    }
    return 6 + this->clientID.length();
}
...
QString SNConnect::getClientID() const
{
    return clientID;
}

void SNConnect::setClientID(const QString &value)
{
    clientID = value;
}
