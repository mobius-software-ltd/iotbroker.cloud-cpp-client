...
SNGWInfo::SNGWInfo(int gwID, QString gwAddress)
{
    this->gwID = gwID;
...
{
    int length = 3;
    if (this->gwAddress.length() > 0) {
        length += this->gwAddress.length();
    }
    return length;
}
...
QString SNGWInfo::getGwAddress() const
{
    return gwAddress;
}

void SNGWInfo::setGwAddress(const QString &value)
{
    gwAddress = value;
}
