...
SNEncapsulated::SNEncapsulated(SNRadius radius, QString nodeID, Message *message)
{
    this->radius = radius;
...
{
    int length = 3;
    if (this->wirelessNodeID.length() > 0) {
        length += this->wirelessNodeID.length();
    }
...
QString SNEncapsulated::getWirelessNodeID() const
{
    return wirelessNodeID;
}

void SNEncapsulated::setWirelessNodeID(const QString &value)
{
